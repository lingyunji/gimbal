...
uint8_t mpu_device_init(void)
{
	...
	uint8_t MPU6500_Init_Data[10][2] = {{ MPU6500_PWR_MGMT_1, 0x80 },     /* Reset Device */ 
																			...
																			{ MPU6500_USER_CTRL, 0x20 },{MPU6500_INT_ENABLE,	0x01}};    /* Enable AUX */ 
	for (i = 0; i < 10; i++)
	{
...
void get_pitchyaw()
{
	MPU6050_Real_Data.Gyro_Z =-mpu_data.gz/32.8f;
	MPU6050_Real_Data.Gyro_Y = -mpu_data.gy/32.8f;
	MPU6050_Real_Data.Gyro_X = -mpu_data.gx/32.8f;
	...
	else if (yaw_temp-last_yaw_temp<=-330)
	{
	 ...
	 {
	 	yaw_angle += MPU6050_Real_Data.Gyro_Z*(Get_Time_Micros() - time)/1000/1000;
